...
export const SettingButton = styled(CardActionArea, {
    shouldForwardProp: prop => prop !== 'active' && prop !== 'simple'
})<{ active: boolean; simple?: boolean }>(({ theme, active, simple }) => ({
    width: '100%',
...
export const OptionsWrapper = styled('div', { shouldForwardProp: prop => prop !== 'grid' })<{ grid: number }>(
    ({ theme, grid }) => ({
        display: 'grid',
        gap: theme.spacing(1),
...
export const SkretchAnim = styled(Box, { shouldForwardProp: prop => prop !== 'active' })<{ active: boolean }>(
    ({ active }) => ({
        display: 'flex',
        width: '24%',
        alignItems: 'center',
...
export const PresetsBtn = styled(CardActionArea, { shouldForwardProp: prop => prop !== 'active' })<{ active: boolean }>(
    ({ theme, active }) => ({
        width: '100%',
        height: 48,
        border: `1px solid ${alpha(theme.palette.grey[500], 0.12)}`,
...
export const Presets = styled(Box, { shouldForwardProp: prop => prop !== 'active' && prop !== 'color' })<{
    active: boolean;
    color: string;
}>(({ active, color }) => ({
    width: 12,
